...
public class MainSurface extends JPanel {
    ...
    // 图片的图层分布
    private final HashMap<Integer, ArrayList<Drawable>> picLayer = new HashMap<>();
    // 修改后的图片的图层分布,这里根据操作分为了两个图层，分别是添加的元素，和删除的元素
    private final HashMap<Integer, ArrayList<Drawable>> addPicLayer = new HashMap<>();
    private final HashMap<Integer, ArrayList<Drawable>> removePicLayer = new HashMap<>();
    // 是否修改过图层
     ...
     */
    public void action() {
        ThreadTool.pool.execute(new OnDrawThread(this));
    }
    ...
    private synchronized void updatePicLayer(int mode, int layerId, Drawable draw) {
        switch (mode) {
                    ...
                    for (Integer id : addPicLayer.keySet()) {
                        for (Drawable d : addPicLayer.get(id)) {
                            ...
                            if (this.picLayer.get(id) == null) {
                                this.picLayer.put(id, new ArrayList<>());
                                updateLayerIds(id);
                            }
            ...
            case CHANGE_MODE_ADD:
                ArrayList<Drawable> al = addPicLayer.computeIfAbsent(layerId, k -> new ArrayList<>());
                al.add(draw);
                changeLayer = true;
                break;
            //删除一个元素
            case CHANGE_MODE_REMOVE:
                ArrayList<Drawable> al1 = removePicLayer.computeIfAbsent(layerId, k -> new ArrayList<>());
                al1.add(draw);
                changeLayer = true;
                break;
        }
        ...
        if (picLayerId.length == 0) {
            ...
            for (int i = 0; i < picLayerId.length; i++) {
                ...
                if (picLayerId[i] > newLayerId) {
                    if (picLayerIdFlag.length - 1 - i >= 0)
                        System.arraycopy(picLayerId, i, picLayerIdFlag, i + 1, picLayerIdFlag.length - 1 - i);
     ...
     * @author Xiloer
     */
    private static class JCanvas implements Canvas {
        /*
         * 绘制画板
         */
        private final BufferedImage canvas = new BufferedImage(GamingInfo.getGamingInfo().getScreenWidth(), GamingInfo.getGamingInfo().getScreenHeight(), BufferedImage.TYPE_4BYTE_ABGR);
        private final JPaint paint = new JPaint(canvas);
        ...
        public static class JPaint implements Paint {
    ...
    public class OnDrawThread extends Thread {
        ...
        private final int drawSpeed;//每次绘制后的休息毫秒数，这个值是根据常量中的绘制帧数决定的
