...
public class FishRunThread extends Thread {
    ...
    private final HeadFish fish;                                                // 当前线程需要控制的领头鱼
    private boolean fishIsOut;                                            // 鱼是否处于屏幕外
    private final int fish_speed = 1000 / Constant.ON_DRAW_SLEEP;                // 鱼绘制速度,这个与屏幕刷新速度一样
    private final float fishrunSpeed;                                        // 鱼旋转长度

    public FishRunThread(HeadFish fish) {
        super();
        this.fish = fish;
        fishrunSpeed = (fish.getFish().getFishInfo().getFishRunSpeed() / (Constant.ON_DRAW_SLEEP * 1F));//这里只是根据速度做了一个比例，让旋转速度看起来比较正常
    }
            ...
            while (isRun && GamingInfo.getGamingInfo().isPause()) {
                for (int[] pathMode : path) {
                    ...
                    if (pathMode[0] == PathManager.PATH_MODE_ROTATE) {
                        // 如果鱼处于第1或第2象限
                        if (fish.getFish_X() <= GamingInfo.getGamingInfo().getScreenWidth() / 2F
                                && fish.getFish_Y() <= GamingInfo.getGamingInfo().getScreenHeight() / 2F
                                || fish.getFish_X() > GamingInfo.getGamingInfo().getScreenWidth() / 2F
                                && fish.getFish_Y() <= GamingInfo.getGamingInfo().getScreenHeight() / 2F) {
                            // 如果鱼头是朝向x的负坐标方向
     ...
     * 设置鱼的外接矩形X,Y的最大值和最小值
     *
     * @param rotateAngle 计算是根据这个参数所旋转的角度来得到对应的X,Y值的
     */
    ...
    private boolean isAtOut() {
        ...
        if (fish.getFishOutlinePoint()[2] > GamingInfo.getGamingInfo().getScreenHeight()
            ...
            // 如果鱼头是朝向x的负坐标方向
                ...
                && fish.getCurrentRotate() >= -270) {
            // 如果鱼处于屏幕左边外
            return fish.getFishOutlinePoint()[1] < 0;
        } else {
            // 如果鱼处于屏幕右边外
            return fish.getFishOutlinePoint()[0] > GamingInfo.getGamingInfo().getScreenWidth();
        }
    }
                    ...
                    while (GamingInfo.getGamingInfo().isGaming()) {
                        if (checkFishAtOut(fish, fishFlag)) {
                            ...
                            GamingInfo.getGamingInfo().getSurface()
                                    .removeDrawablePic(fishFlag.getFishInfo().getFishInLayer(), fishFlag);
    ...
    private boolean checkFishAtOut(HeadFish headFish, Fish fish) {
        return headFish.getFish_X() - fish.getDistanceHeadFishX() + fish.getPicWidth() < 0 || headFish.getFish_X() - fish.getDistanceHeadFishX() > GamingInfo.getGamingInfo().getScreenWidth()
