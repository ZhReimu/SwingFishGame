...
public class ShotThread extends Thread {
    ...
    private final float ammoRotateX;
    private final float ammoRotateY;
    private float speed_x;                                                        // 取一个近似值，代表每帧移动的像素数
    private float speed_y;
    private final Ammo ammo;                                                            //子弹
    ...
    public ShotThread(float targetX, float targetY, Ammo ammo, float fromX, float fromY) {
        ...
        currentX = fromX;
        currentY = fromY;
        ammoRotateX = ammo.getPicWidth() / 2F;
        ammoRotateY = ammo.getPicHeight() / 2F;
        ...
        float time = len / (Constant.AMMO_SPEED / (Constant.ON_DRAW_SLEEP * 1F));        // 计算目标与始发之间子弹需要行走的帧数
    ...
    public void run() {
        try {
            //如果子弹帧数多于1，就播放子弹动画
            if (ammo.getAmmoPicLength() > 1) {
                ThreadTool.pool.execute(this.playAmmoAct());
            }
            // 计算子弹需要的旋转角度
            float angle = Tool.getAngle(targetX, targetY, currentX, currentY);
            AmmoParticleEffect effect = ManagerFactory.getInstance(ParticleEffectManager.class).getAmmoEffect();
            ...
            if (GamingInfo.getGamingInfo().isGaming()) {
                while (GamingInfo.getGamingInfo().isPause()) {
                    ...
                    try {
                        // 子弹绘制速度,这个与屏幕刷新速度一样
                        int ammo_speed = 1000 / Constant.ON_DRAW_SLEEP;
            ...
            try {
                if (GamingInfo.getGamingInfo().isGaming()) {
                    while (GamingInfo.getGamingInfo().isPause() && ammoActIsRun) {
                        ...
                        picIndex++;
                        if (picIndex == ammo.getAmmoPicLength()) {
                            picIndex = 0;
                        }
                        Thread.sleep(200);
                    }
