...
public class Fish extends DrawableAdapter {
    ...
    private final int[] fishOutlinePoint = new int[4];        //鱼的外接矩形，x的最小值，最大值，Y的最小值，最大值
...
//		System.out.println("鱼被捕捉了，但是没有捕捉到");
     ...
     */
    public void onCaught(Ammo ammo, final float targetX, final float targetY) {
        this.setAlive(false);
        new Thread(() -> {
            try {
                ...
                Thread.sleep(1800);
                //调用增加分数方法
                ManagerFactory.getInstance(ScoreManager.class).addScore(getFishInfo().getWorth(), fishX, fishY);
                Thread.sleep(200);
                ...
                Log.e("Fish_onCaught", e.toString());
            }
        }).start();
    }
