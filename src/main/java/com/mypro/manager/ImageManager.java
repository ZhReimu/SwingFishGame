...
import com.mypro.model.ImageConfig;
 ...
 * @author Xiloerfan
 */
public class ImageManager extends IManager {
...
//				getFishActImage(xml, src);
    ...
    public HashMap<String, Bitmap> getImagesMapByImageConfig(ImageConfig config, float proportion) {
        HashMap<String, Bitmap> allAct = new HashMap<>();
        try {
    ...
    public Bitmap getBitmapByAssets(String imageName) throws Exception {
        try {
            return new Bitmap(ImageIO.read(getResourceFileByName(imageName)));
        } catch (IOException e) {
            LogTools.doLogForException(e);
    ...
    private URL getResourceFileByName(String fileName) {
